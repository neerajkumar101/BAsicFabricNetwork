...
func init_document(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	if len(args) != 6 {
		fmt.Println("initDocument(): Incorrect number of arguments. Expecting 6 ")
		return shim.Error("intDocument(): Incorrect number of arguments. Expecting 6 ")
	}
...
func CreateDocumentObject(args []string) (Document, error) {
	...
	if len(args) != 6 {
		fmt.Println("CreateDocumentObject(): Incorrect number of arguments. Expecting 6 ")
		return myDocument, errors.New("CreateDocumentObject(): Incorrect number of arguments. Expecting 6 ")
	}

	myDocument = Document{args[0], args[1], args[2], args[3], args[4], args[5], time.Now().Format("20060102150405")}
...
func update_document(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	updatedDocument := Document{dat.DocumentChannel, dat.userUUID, dat.DocumentHash, document_Status, dat.OwnerPublicKey, dat.VerifierPublicKey, time.Now().Format("20060102150405")}
